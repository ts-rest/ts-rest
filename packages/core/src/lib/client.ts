...
export interface OverrideableClientArgs {
  baseUrl: string;
  /* @deprecated Use `defaultFetchOptions.credentials` instead */
...
export const fetchApi = (options: FetchApiOptions) => {
  ...
  let fetcherArgs: ApiFetcherArgs = {
    ...
    fetchOptions: {
      ...
      ...clientArgs.defaultFetchOptions,
      ...fetchOptions,
    },
    ...(fetchOptions?.signal && { signal: fetchOptions.signal }),
    ...(fetchOptions &&
      'cache' in fetchOptions && { cache: fetchOptions.cache as any }),
    ...(fetchOptions &&
      'next' in fetchOptions && { next: fetchOptions.next as any }),
  };
