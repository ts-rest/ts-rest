...
Many teams who use `ts-rest` tend to put their contract in a shared place, whether that's a shared module in a monorepo, a shared folder in a single repo or a external package.
