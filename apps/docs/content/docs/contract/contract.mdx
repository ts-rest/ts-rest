...
```typescript title="combining-contracts.ts"
import { initContract } from '@ts-rest/core';
const c = initContract();
// ---cut---
const postContract = c.router({
  ...
  createPost: {
    method: 'POST',
    path: '/posts',
    body: c.type<{ title: string; content: string }>(),
    responses: {
      201: c.type<{ id: string; title: string }>(),
    },
  },
});
...
export const contract = c.router({
  getPosts: {
    ...
    metadata: { role: 'guest', cache: true } as const,
  },
  createPost: {
    method: 'POST',
    path: '/posts',
    ...
    metadata: { role: 'admin', rateLimit: 10 } as const,
  },
});
```
...
Configure how your contract behaves with these options.

### Common Responses

Define shared response schemas across all routes in your contract, typically for error responses.

<Tabs items={['Zod', 'Valibot', 'Arktype']}>
  <Tab value="Zod">
```typescript  title="common-responses.ts"
import { initContract } from '@ts-rest/core';
import { z } from 'zod';
const c = initContract();
// ---cut---
...
export const contract = c.router(
  ...
  },
  {
    commonResponses: {
      404: v.object({
        message: v.literal('Not Found'),
        reason: v.string(),
      }),
      500: c.otherResponse({
        contentType: 'text/plain',
        body: v.literal('Server Error'),
      }),
    },
  }
);
...
```typescript title="strict-status-codes.ts"
import { initContract } from '@ts-rest/core';
const c = initContract();
// ---cut---
export const contract = c.router(
...
```typescript title="path-prefix.ts"
import { initContract } from '@ts-rest/core';
const c = initContract();
// ---cut---
...
```typescript title="intellisense.ts"
import { initContract } from '@ts-rest/core';
import { z } from 'zod';
const c = initContract();
// ---cut---
