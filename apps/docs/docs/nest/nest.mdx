...
You can disable the validation of these components if you wish to perform this validation manually or handle the error differently.

Using `@TsRest()` decorator
```typescript
import { TsRest } from '@ts-rest/nest';
...
Or only for for a specific route
```typescript
import { TsRestHandler, tsRestHandler } from '@ts-rest/nest';
