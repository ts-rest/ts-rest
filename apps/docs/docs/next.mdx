...
import { createSingleUrlNextRoute } from '@ts-rest/next';

export default createSingleUrlNextRouter(api.posts.createPost, async (args) => {
  const newPost = await posts.createPost(args.body);
...
`createSingleUrlNextRoute` is a function that takes a single contract route and a handler, and creates a `nextApiHandler`
