...
:::info
Our OpenAPI generator uses `@anatine/zod-openapi` to geneate open-api docs for your Zod schemas, so make sure to use Zod everywhere on your API to get the best experience.
:::
...
## Enabling `operationId`'s (Reccomended!)
