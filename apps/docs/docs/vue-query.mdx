...
To use the query client in your Vue app, you'll need to initialise the `react-query` client in your app.
...
The design philosophy of `@ts-rest/vue-query` is to make it as easy as possible to use `react-query` with `@ts-rest`. This means that the `useQuery` and `useMutation` hooks are as close to the original `react-query` hooks as possible, as such we don't abstract away from the query keys or the options. Only the query function itself!
